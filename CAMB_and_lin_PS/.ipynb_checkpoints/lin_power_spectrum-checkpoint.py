...
def load_transfer_from_file(file_path):
    ...
    transfer_dic = {
        ...
        'transfer_massless-neutrino': np.loadtxt(file_path, unpack=True,
                                                 usecols=components_dic['massless_neutrino']),
        'transfer_massive-neutrino': np.loadtxt(file_path, unpack=True,
                                                usecols=components_dic['massive_neutrino']),
    }
    return transfer_dic
...
def func_power_spec_dic(file_path, cosmo_dic):
    ...
    power_spec_dic['cold'] = transfer_to_PS(transfer_dic['k'],
                                            (cosmo_dic['Omega_b_0']*transfer_dic['transfer_baryon'] + \
                                             ...
                                             cosmo_dic)
    power_spec_dic['power_massive_neutrino'] = transfer_to_PS(transfer_dic['k'], transfer_dic['transfer_massive-neutrino'], cosmo_dic)
...
def func_power_spec_interp_dic(power_spec_dic, cosmo_dic):
    ...
    power_spec_interp_dic['power_massive_neutrino'] = func_PS_interpolate(power_spec_interp_dic['k'], power_spec_dic['k'], power_spec_dic['power_massive_neutrino'])
