...
def func_rho_m_comp(cosmo_dic, Omega_0): 
    """
    Omega_0 depends on the consinderd matter component 
    returns backrground density of given component in solar_mass/Mpc^3*h^2
    """
    grav_const = const.G.to('km**2*Mpc/(Msun*s**2)').value
    rho_crit_0 = 3. * 100**2 / (8. * np.pi * grav_const)
    
    return Omega_0 * (1+cosmo_dic['z'])**3 * rho_crit_0
