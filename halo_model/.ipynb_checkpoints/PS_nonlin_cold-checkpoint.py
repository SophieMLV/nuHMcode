...
def func_non_lin_PS_matter(M, k, PS, k_sigma, PS_sigma, cosmo_dic, hmcode_dic, Omega_0, Omega_0_sigma, LCDM=True, 
                           alpha = False, eta_given = False, nu_one_halo=False, one_halo_damping = False, two_halo_damping = False):    
    """ 
    The cold halo model as in my masterthesis eq. 4.9 with (if set to True) the modifications of HMcode2020 https://arxiv.org/abs/2009.01858
    ...
    """     
    dens_profile_arr = func_dens_profile_kspace(M, k, k_sigma, PS_sigma, cosmo_dic, hmcode_dic, Omega_0, Omega_0_sigma, LCDM=LCDM, eta_given = eta_given)
    halo_mass_func_arr = func_halo_mass_function(M, k_sigma, PS_sigma, cosmo_dic, Omega_0, Omega_0_sigma, LCDM=LCDM)
    halo_bias_arr = func_halo_bias(M, k_sigma, PS_sigma, cosmo_dic, Omega_0_sigma, LCDM=LCDM)
    
    integrand_arr_one = M[:, None]**2 * halo_mass_func_arr[:, None] * dens_profile_arr**2 
    #no neutrinos in halos
    if nu_one_halo == True:
        f_ax = cosmo_dic['Omega_nu_0']/cosmo_dic['Omega_m_0']
    ...
    if two_halo_damping == True:
        ...
        summand2 = func_dens_profile_kspace(np.min(M), k, k_sigma, PS_sigma, cosmo_dic, hmcode_dic, Omega_0, Omega_0_sigma, LCDM=LCDM, eta_given = eta_given) \
