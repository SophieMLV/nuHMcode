...
def func_z_formation(M, k_sigma, PS_sigma, cosmo_dic, Omega_0_sigma, LCDM=True, f = 0.01):
    """
    ...
    def func_find_root(x, Mass):
        nu = func_nu(f*Mass, k_sigma, PS_sigma, cosmo_dic, Omega_0_sigma, LCDM=LCDM)
...
def func_conc_param(M, k_sigma, PS_sigma, cosmo_dic, Omega_0_sigma, LCDM=True):
    """
    k_sigma is in units of h/Mpc, PS_sigma in (Mpc/h)^3 and M in solar_mass/h
...
def func_dens_profile_kspace(M, k, k_sigma, PS_sigma, cosmo_dic, hmcode_dic, Omega_0, Omega_0_sigma, LCDM=True, eta_given = False):
    """
    ...
    if eta_given == True:
        eta = hmcode_dic['eta']
        nu = func_nu(M, k_sigma, PS_sigma, cosmo_dic, Omega_0_sigma, LCDM=LCDM)
    else:
        eta = np.array([0.]) 
        nu = 1.
    
    R_vir = func_r_vir(M, cosmo_dic, Omega_0)
    concentration = func_conc_param(M, k_sigma, PS_sigma, cosmo_dic, Omega_0_sigma, LCDM=LCDM) / (nu**eta)
...
def func_delta_char(M, k_sigma, PS_sigma, cosmo_dic, Omega_0, Omega_0_sigma, LCDM=True, eta_given = False): 
    """
...
def NFW_profile(M, r, k_sigma, PS_sigma, cosmo_dic, hmcode_dic, Omega_0, Omega_0_sigma, LCDM=True, eta_given = False):
    """
    ...
    concentration = func_conc_param(M, k_sigma, PS_sigma, cosmo_dic, Omega_0_sigma, LCDM=LCDM) / (nu**eta)
    normalisation = func_delta_char(M, k_sigma, PS_sigma, cosmo_dic, Omega_0, Omega_0_sigma, LCDM=LCDM, eta_given = eta_given) 
